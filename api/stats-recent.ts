...
// ============ GITHUB API ============
async function fetchRecentStats(username: string, token: string, limit: number = 50) {
  const reposQuery = `
    ...
    try {
      ...
      for (const batch of batches) {
        ...
        for (const result of results) {
          ...
          totalCommits++;
          if (totalCommits >= limit) return; // Early exit
        }
        if (totalCommits >= limit) break;
      }
    } catch { }
  };
...
function createStatsSVG(username: string, stats: any, theme: Theme): string {
  ...
  const statItems = [
    ...
    { label: 'Commits', value: stats.totalCommits.toString(), color: theme.title },
  ...
  <text x="20" y="28" fill="${theme.title}" font-size="16" font-weight="600">${username}'s Recent Stats</text>
  <text x="${width - 20}" y="28" text-anchor="end" fill="${theme.text}" font-size="10" opacity="0.5">Last ${stats.totalCommits} Commits</text>
  ${rows}
</svg>`;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const stats = await fetchRecentStats(username, process.env.GITHUB_TOKEN, parseInt(limit as string, 10));
